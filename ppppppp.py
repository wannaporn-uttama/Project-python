data_ = open('D:\\ProJ.txt','w')
import os
os.system('cls')
count = []
data_ = open('D:\\ProJ.txt','r')
datastudent = {
...
def save_d():
    data_ = open('D:\\ProJ.txt','w')
    i = 0
    for x in datastudent:
        z = datastudent.get(x)
        n = clas.get(x)
        d = count.count(x)
        i += 1
        data_.write('{0: <3}.'.format(i))
        data_.write(x+'  ')
        data_.write(z)
        data_.write('{0:<10}'.format('เข้าใช้ '))
        data_.write('{0: <4}'.format(d))
        data_.write(' ครั้ง\n')
    e = len(count)
    f = str(e)
    data_.write('รวมทั้งหมด ----> ')
    data_.write(f)
    data_.write(' ครั้ง ')
